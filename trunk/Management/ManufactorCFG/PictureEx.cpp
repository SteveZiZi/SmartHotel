...
//新方法的代码区
	...
	//定义实际的大小
	if(nPaintWidth > m_PictureSize.cx)   //框比图像大
	{
	   RealRect.left = (nPaintWidth-m_PictureSize.cx)/2;
       RealRect.right = RealRect.left + m_PictureSize.cx;
	}
    else
	{
	   RealRect.left = 0;
       RealRect.right = m_PictureSize.cx;	
	}
    //==============================================================
	if(nPaintHeight > m_PictureSize.cy)   //框比图像大
	{
	   RealRect.top = (nPaintHeight-m_PictureSize.cy)/2;
       RealRect.bottom = RealRect.top + m_PictureSize.cy;
	}
    else
	{
	   RealRect.top = 0;
       RealRect.bottom = m_PictureSize.cy;
	}

    ::BitBlt(dc.m_hDC, RealRect.left,RealRect.top,RealRect.right,RealRect.bottom,
