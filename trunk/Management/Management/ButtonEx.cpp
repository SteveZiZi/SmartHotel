...
//	pDC->DrawText(sContent,tRect,0);	
	pDC->TextOut(Rect.left+8,Rect.top+pSize.cy+15,sContent);
...
void CButtonEx::SetControlSource(PWCHAR lPath, PWCHAR lFocusPath, float Flag)
{
   //先加载正常的图片
   ...
   pSize.cy = (long)(m_pImage->GetHeight() * Flag);
   pSize.cx = (long)(m_pImage->GetWidth() * Flag);
...
void CButtonEx::DrawActivePicture(CDC *pDC)
{
   ...
   graphics.DrawImage(m_Image,pRect.left,pRect.top,pSize.cx,pSize.cy);//
